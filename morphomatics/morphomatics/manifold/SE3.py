...
class SE3(Manifold):
    ...
    class GroupStructure(LieGroup):
        ...
        def action(self, g, x):
            ...
            return jnp.matmul(self._M.get_so3(g).squeeze(), x.squeeze()) + self._M.get_r3(g).squeeze()
    ...
    class CanonicalRiemannianStructure(Metric):
        ...
        def curvature_tensor(self, S, X, Y, Z):
            ...
            return V.at[:, :3, :3].set(
                self._SO3.connec.curvature_tensor(S[:, :3, :3], X[:, :3, :3], Y[:, :3, :3], Z[:, :3, :3]))
        ...
        def projToGeodesic(self, S, P, Q, max_iter=10):
            ...
            Pi = jnp.zeros_like(S)
            Pi = Pi.at[:, :3, :3].set(
                self._SO3.metric.projToGeodesic(S[:, :3, :3], P[:, :3, :3], Q[:, :3, :3], max_iter))
...
def expm(X):
    ...
    V = jnp.eye(3) \
        ...
        + jnp.where(theta < 1e-6, 1 / 6 - theta2 / 120 + theta2 ** 2 / 5040,
                    (theta - jnp.sin(theta)) / (theta2 * theta)) \
        * (X[:, :3, :3] @ X[:, :3, :3])
